...
experimental_filename = 'Oncotripsy\Exp_CS_300kHz_PD.xlsx';
...
for i=indexes
    ...
    while toc(h_tic) < w.exposure_duration
        ...
        elseif zigzag == 1
            zigzag = 2;
            if ~NoStage_Mode; params = sub_Stage_Move_To(params, w.coor + params.Stages.Origin + 0.5 * zigzap_disp); end
        elseif zigzag == 2
            zigzag = 3;
            if ~NoStage_Mode; params = sub_Stage_Move_To(params, w.coor + params.Stages.Origin + 1.0 * zigzap_disp); end
        elseif zigzag == 3
            zigzag = 4;
            if ~NoStage_Mode; params = sub_Stage_Move_To(params, w.coor + params.Stages.Origin + 1.5 * zigzap_disp); end
        else
            zigzag = 0;
            if ~NoStage_Mode; params = sub_Stage_Move_To(params, w.coor + params.Stages.Origin + 2.0 * zigzap_disp); end
        end
