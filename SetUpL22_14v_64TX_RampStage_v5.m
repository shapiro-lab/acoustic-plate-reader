...
P.savepath = [pwd '\Data\210211_Test\'];
P.saveDirName = 'Test1';
P.saveDirName = [P.saveDirName '_' datestr(now, 'yyyy-mm-dd@HH-MM-SS')];

P.seed = 4:1:6; % Voltge ramp range
PostCollapse = P.seed; % post-collapse voltage(s)
P.Focus = 5e-3; % focus of parabolic beam [m]

P.xLines = 3; % number of scanned lines in the x-direction
...
P.EqTime2 = 5; % Parabolic collapse time
P.EqTime3 = 2;
...
Resource.RcvBuffer.rowsPerFrame = P.numRays*maxBufSizePerAcq;
...
Event(n).rcv = 0;       % no Rcv
Event(n).recon = 0;     % no Recon
Event(n).process = 0;%nproc_motorReturnXReturnZ;   % Back to Origin
Event(n).seqControl = 0;
n = n + 1;
...
m = 1;
% - Angle change
% UI(m).Control = {'UserB1','Style','VsSlider','Label','Angle',...
%     'SliderMinMaxVal',[P.Angles(1),P.Angles(end),P.alpha],...
%     'SliderStep',[1/(length(P.Angles)-1),1],'ValueFormat','%2.1f',...
%     'Tag','angleSldr'};
% UI(m).Callback = text2cell('%AngleCallback');
% m = m + 1;
...
%AngleCallback - angle change
% P = evalin('base','P');
% Resource = evalin('base','Resource');
% nAngleEvent = evalin('base','nAngleEvent');
% 
% if (~strcmpi(P.pulseShape,'parabola'))
%     [~,iAngle] = min(abs(P.Angles - UIValue));
%     P.alpha = P.Angles(iAngle);
%     Resource.Parameters.startEvent = nAngleEvent(1+strcmpi(P.code,'AM'),iAngle);
%     
%     % TODO: change slider and value if UIValue ~= P.alpha
% %     angleSldr = findobj('Tag','angleSldr');
% %     set(angleSldr,'Value',hv);
% %     angleValue = findobj('Tag','angleValue');
% %     set(angleValue,'String',num2str(hv,'%.1f'));
% 
%     % Update voltage to make appropriate for angle
% %     [A,D] = meshgrid(P.HP.angle,P.HP.depth);
% %     P.hv = interp2(A,D,P.HP.HighVoltage,P.alpha,P.wellDepth);
...
if P.ramp && (etime(clock,P.t1) > P.EqTime)
    if P.Collapse 
        ...
        if P.rampi == 1
            ...
            ScanName = sprintf('_X%02d_Z%02d_',P.ScanCount(1),P.ScanCount(2));
            P.dir_save = [P.savepath P.saveDirName ScanName '\'];
            mkdir(P.dir_save)
        end
        if P.RForImg(1)
            if (strcmpi(P.pulseShape,'parabola'))
...
if P.ScanCount(3) && P.ScanCount(1) == P.xLines
    if P.ScanCount(2) < P.zLines
        ...
        pause(0.5+(P.xLines-1)*(P.xDist/1000)/params.Stages.step_distance/params.Stages.Speed)
