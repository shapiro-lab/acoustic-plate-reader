...
sample_depth = [2 8]; % display/sample dpeth range in mm
DisplayMode = 0; % How to display images: 0 = show nothing, 1 = always shwo images + ROIs, 2 = only show images + ROIs with inconfident ROI selection
testfilter = 0; % 0 or 1 to display the thresholding filter
savedata = 0; % 0 or 1 to save processed data
...
TemplateMode = 1; % ROI template to use: 1 = filled wells, 2/3 = empty wells with thiner (2) or thicker (3) well wall, use 1 as default
...
zcorrection = zeros(1,total_n);% initialize for ROI correction in z dimension (correction in the unit of voxel count, ~0.0124 mm/count)  
...
for wellInd = 1:64 
    if ~skip(wellInd)
        ...
        c = xcorr2(WellTemplate,Imt_f2); % Calculate 2D cross correlation between images and centered ROI template 
        ...
        zoffset = zpeak - size(Imt_f2,1); % Convert max correlation location to index offset in z dimension
        xoffset = xpeak - size(Imt_f2,2); % Convert max correlation location to index offset in z dimension
