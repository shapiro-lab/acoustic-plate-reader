...
%         fprintf(params.SG.visaObj,':OUTPUT:SYNC 1');
    ...
    elseif strcmp(params.SG.Instrument, 'BKP')
        ...
        s = 'C1:OUTP LOAD,50; C2:OUTP LOAD,50; C2:SYNC ON;';        
        
        s = [s 'C2: BSWV WVTP,SINE,OFST,0,PHSE,0,'];
        s = [s sprintf('FRQ,%1.5e,', params.SG.Waveform.frequency)];
        s = [s sprintf('AMP,%1.5e; ', params.SG.Waveform.voltage)];
                
        s = [s 'C2: BTWV STATE,ON,GATE_NCYC,GATE,TRSR,EXT,PLRT,POS; '];
                
        s = [s 'C1: BSWV WVTP,PULSE,AMP,3,OFST,1.5,PHSE,0,'];
        ...
        s = [s 'C1: BTWV STATE,ON,GATE_NCYC,NCYC,TRSR,MAN,' sprintf('TIME,%1.0f; ',params.SG.Waveform.repeats)];
