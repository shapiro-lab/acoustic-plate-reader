...
%% Mandatory Cable Setup

% Using BK Precision signal generator, parameters not optimized for Tabor

% Cabling setup for safe operation
%   SG Ch 2     -> SG Ext Trig In
%   SG Ch 1     -> Amp In
%   Amp out     -> Transducer

% To observe waveform on oscilloscope
%   SG Ch 2     -> Scope Ch 1 set on 1 Mohm (set trigger to Ch 1)
%   SG Ch 1     -> Scope Ch 2 set on 1 Mohm
%   Hydrophone  -> Scope Ch 3 set on 50 Ohm

% Also, remember to update amplifier settings in sub_AllSettings prior to
...
experimental_filename = 'Oncotripsy\Exp_CQ_670kHz_PD_1.09MPa.xlsx';
% This is the filename of the excel document that we are loading to run
...
for i = 1:24
    ...
    if w.exposure_duration > 0 && w.peak_to_peak > 0 && w.freq > 0 
       string = sprintf('%s kHz\n%s V\nPD %s s\n DC %s \nDur %s s', ...
           num2str(w.freq/1000), ...
    ...
    if isempty(findstr(version,'R2016b'))
        
        % Matlab version not R2016b
        fill(zm + wellz, xm + wellx, well_c)
        hold('on')

        params.GUI.h_WellProgressSqare(i) = ...
            fill(zm + wellz * 0, xm + wellx, [0.6 0.6 1]);

        plot(zm + wellz, xm + wellx, 'k-')
        set(gca, 'Ydir','reverse')
        axis('equal')

        text(zm + wellr, xm + wellr, string, ...
            'FontSize', 8, ...
...
for i=indexes
    ...
    th = 0;
    dth = pi/8;
    r = 0.0005 / params.Stages.step_distance;
    
    while toc(h_tic) < w.exposure_duration
        if params.GUI.Flags.Quit == 1; break; end
        
        if ~NoStage_Mode; 
            if 1==1;
                % Only rotate around well B2
                ds = [0 0 0];
                ds(params.Stages.x_motor) = r*sin(th);
                ds(params.Stages.z_motor) = r*cos(th);
                params = sub_Stage_Move_To(params, ds + w.coor + params.Stages.Origin); 
            else
                pause(0.005);
            end
        end
...
%         disp(th);
    ...
    if ~NoScope_Mode;
        params = sub_Scope_Readout_All(params);
        params.Plate.Wells(i).Waveform.Ch2 = params.Scope.Ch2.YData;
        params.Plate.Wells(i).Waveform.Ch3 = params.Scope.Ch3.YData;
        params.Plate.Wells(i).Waveform.t = params.Scope.Ch3.XData;
    end
