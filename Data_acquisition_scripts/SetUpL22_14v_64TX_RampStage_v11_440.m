...
P.AcousticCollapse = 2; % 1 = normal ramp, 2 = hold voltage P1 and change voltage P2 (fixed at parabolic Bmode)
AcousticCollapseImagingVoltage = 25; % only use for acoustic collapse mode, imaging voltage

% params = sub_AllSettings('ZJScanTest');
% params = sub_Stage_Initialize(params);

P.savePath = [pwd '\Data\220725\'];
P.saveDirName = 'A-lib_P1-4_R3_B-lib_P1-4_R4_stable-37C';

Initial_Pos = 'A1'; %Starting point
Initial_PlatePos = [1 1];


P.seed = [10:2:30]; % Voltge ramp range, min voltage = 1.6V 
PostCollapse = [10:2:30]; % post-collapse voltage(s)
...
P.pNum = [2 4]; % number of plates [row column] in [x z] direction (max [3 4])
