...
P.savePath = [pwd '\Data\220412\'];
P.saveDirName = 'A-ctrls_stable-37C';

Initial_Pos = 'A1'; %Starting point
Initial_PlatePos = [1 1];


P.seed = [10:25]; % Voltge ramp range, min voltage = 1.6V 
PostCollapse = [10:25]; % post-collapse voltage(s)
...
P.pNum = [1 1]; % number of plates in [x,z] direction (max [3 4])
...
P.saveDirName1 = [P.saveDirName '_P_' num2str(P.PlateCount(1)) '_' num2str(P.PlateCount(2))];
        ...
        if P.ScanCount(2) < P.zLines            
            ...
            if all(P.PlateCount(1:2) == P.pNum)
                ...
                if XD_temp
                    ...
                    pause(1+(abs(XD_temp)/1000)/params.Stages.step_distance/params.Stages.Speed)
                    disp('Motor Plate X')
                end
                ...
                pause(1+(abs(ZD_temp)/1000)/params.Stages.step_distance/params.Stages.Speed)
                disp('Motor Plate Z')
